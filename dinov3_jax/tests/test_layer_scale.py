...
def test_layer_scale_forward():
    ...
    gamma_pt = layer_pt.gamma.detach().numpy()
    cx[layer_jax.gamma] = jnp.array(gamma_pt)
    ...
    params_2 = {}
    cx_2 = Context(params_2, key)
    layer_jax_2.setup(cx_2)
    
    # Copy weights
    gamma_pt_2 = layer_pt_2.gamma.detach().numpy()
    cx_2[layer_jax_2.gamma] = jnp.array(gamma_pt_2)
    ...
    output_jax_2 = layer_jax_2(cx_2, x_jax)
...
def test_layer_scale_gradient():
    ...
    # Copy PyTorch weights to JAX
    gamma_pt_init = layer_pt.gamma.detach().numpy()
    cx[layer_jax.gamma] = jnp.array(gamma_pt_init)
    ...
    # Compute gradients with respect to params and input
    loss_jax, grads_jax = jax.value_and_grad(loss_fn)(cx.params, x_jax, target_jax)
    gamma_grad_jax = np.array(grads_jax[layer_jax.gamma.name])
