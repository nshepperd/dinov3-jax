...
def test_self_attention_forward():
    ...
    # Compare outputs (relaxed tolerance due to flash attention fp16 casting)
...
def test_self_attention_with_rope():
    ...
    output_jax = layer_jax(x_jax, rope=rope_jax)

    # Compare (relaxed tolerance due to flash attention fp16 casting)
...
def test_self_attention_gradient():
    ...
    # Compare parameter gradients (relaxed tolerance for attention gradients with flash attention)
    qkv_grad_jax = np.array(grads_jax.qkv.weight)
    qkv_grad_pt = layer_pt.qkv.weight.grad.numpy()
    np.testing.assert_allclose(qkv_grad_jax, qkv_grad_pt, rtol=1e-2, atol=1e-2)
